function extractWB(x)
itr = num2str(x);
disp(itr);
caffe.reset_all();
clearvars -except itr; 
close all;

%% settings
folder = '';
model = [folder 'SRCNN_mat.prototxt'];
weights = ['trained_models/SRCNN_iter_',itr,'.caffemodel'];
disp(weights);
%weights = join(weight, '');
%weights = [folder 'SRCNN_iter_1000.caffemodel'];
savepath = ['MATfiles/x3_' itr '.mat'];
%savepath = [folder 'x3_1000.mat'];
layers = 3;
